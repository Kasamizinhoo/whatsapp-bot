  ...
  client.onMessage(async (message) => {
    ...
    if (selectedOption) {
      ...
      if(!validateInput(selectedOption)) {
        client.sendText(message.from, "Opção inválida, selecione número válido");
        return;
      }
    ...
    if (currentState.transferido) {
      ...
      if (option.handler === "lista") {
        ...
        if(message.body !== "lista") {
          client.sendText(message.from, "Digite apenas 'lista' para reiniciar");
          return;
        }
